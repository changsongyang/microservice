package org.study.common.mq.producer;
...
import org.study.common.mq.processor.AfterSendAbstractProcessor;
import org.study.common.mq.processor.AfterSendLoggerProcessor;
...
public class Producer {
    ...
    public void initProducer() throws Exception{
        ...
        if(Boolean.FALSE.equals(isSupportTransaction)){
            ...
            if(localTransactionProcessor == null){
                throw new BizException("localTransactionProcessor is null");
            }
     ...
     *      日志、redis、数据库中作为备份，这个功能可通过继承{@link AfterSendAbstractProcessor}并覆写相关方法来实现
     ...
     * @throws BizException 这在多数情况下表示消息是发送失败的，但也有特殊情况：超时，即发送端等待broker端的响应超时，这种情况下，消息是有可能发送成功的
     */
    public <T> Boolean send(PMessage<T> msg) throws BizException {
     ...
     * @return
     */
    public <T> Boolean sendBatch(String topic, List<PMessage<T>> msgList) throws BizException {
        int messageSize = 0;
        List<Message> messageList = new ArrayList<>();
        for(PMessage msg : msgList){
            ...
            if(this.isMessageOverSize(messageSize, maxMessageSize)){
                throw new BizException("批量消息总体内容过大");
            }
     ...
     * @return
     */
    public <T> Boolean sendOrderly(PMessage<T> msg) throws BizException {
        Message message = MessageUtil.convertMessage(msg);
        this.validMessage(message, msg);
     ...
     * @throws BizException
     */
    public <T> Boolean sendTransaction(PMessage<T> msg) throws BizException {
        if(! isSupportTransaction){
            throw new BizException("当前生产者不支持事务，请在Bean配置时设置 isSupportTransaction = true");
        }else if(StringUtil.isEmpty(msg.getMsgEvent())){
            throw new BizException("msgEvent不能为空");
        }
    ...
    private void validMessage(Message message, PMessage pMessage){
        if(StringUtil.isEmpty(message.getTopic())){
            throw new BizException("topic不能为空");
        }else if(StringUtil.isEmpty(message.getTags())){
            throw new BizException("tags不能为空");
        }else if(StringUtil.isEmpty(message.getKeys())) {
            throw new BizException("key不能为空");
        }else if(isMessageOverSize(message.getBody().length, maxMessageSize)){
            throw new BizException("消息体内容过大");
        }else if(StringUtil.isEmpty(pMessage.getMsgEvent()) || pMessage.getMsgEvent() == 0){
            throw new BizException("msgEvent不能为null或0");
        }
    }
    ...
    public void setIsSupportTransaction(Boolean isSupportTransaction) {
        if(isSupportTransaction == null){
            throw new BizException("isSupportTransaction不能为NULL");
        }else if(this.isSupportTransaction != null){
            throw new BizException("初始化已完成，不可再设置isSupportTransaction的值");
        }
